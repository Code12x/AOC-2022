...
fn main() {
    ...
    // Start poi
    let start_cords = match map.find_mark('E') {
        Ok(cords) => cords,
        Err(e) => panic!("Error finding santa {}", e),
    };
    map.map[start_cords.0][start_cords.1].height = 'z';

    // Setup the paths and give it the inital value of santa's path from santa_cords
    let mut paths: Vec<Path> = Vec::new();
    paths.push(Path {poi: map.get_poi(start_cords.0, start_cords.1), moves_count: 0});
